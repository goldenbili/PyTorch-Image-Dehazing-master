...
class dehazing_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        return list_tensor_yuv444, list_tensor_yuv420, list_tensor_rgb, bl_num_width, bl_num_height
...
class dehazing_loader(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # load image
        data_orig = Image.open(data_orig_path)
        bl_num_width = data_orig.width / self.bkW
        bl_num_height = data_orig.height / self.bkH
        if self.resize:
            data_orig = data_orig.resize((480, 640), Image.ANTIALIAS)
        ...
        # data_orig to yuv444, yuv420
        data_yuv444, data_yuv420 = bgr2yuv(data_orig, self.bkW, self.bkH)
        list_tensor_yuv444 = []
        list_tensor_yuv420 = []
        for yuv444 in data_yuv444:
            yuv444 = (np.asarray(yuv444) / 255.0)
